            ...
            <div class="row">
              <div class="col-md-12">
                          ...
                          <div class="col-lg-12">
                            <div class="element-wrapper">
                              <div class="element-box">
                                ...
                                <input type="hidden" name="selectplayer" value="<?php echo isset($playerid)?$playerid:''; ?>" />
                                <input type="hidden" name="mandate_editid" value="<?php echo isset($mandate_check)?$mandate_check->_Mandate_ID:''; ?>" />
                                  <div class="row">

                                    <?php
                if(isset($ismaindate) AND count($ismaindate)>1){
                ?>
                    <div class="col-sm-6 col-md-4">
                        <div class="form-group">
                        <label for="">Select Agent</label>
                        <?php //print_r($myplayers);  
                        //echo $mandate_check->_Mandate_ID; ?>
                        <select id="selectagent" name="selectagent" class="custom-select form-control" required  >
                        <option value="">Select Agent </option>
                         <?php 
                            foreach ($ismaindate as $agent) {
                            ?>
                              <option value="<?php echo $agent->_AgentID;  ?>"
                              <?php if(isset($mandate_check) AND $mandate_check->_Agent_ID == $agent->_AgentID)
                                 { 
                                  echo "SELECTED"; 
                                 }
                                 else if($this->uri->segment(2) == $agent->_AgentID)
                                 { echo "SELECTED";
                                 } ?>
                              ><?php echo $agent->_Firstname.' '.$agent->_Lastname; ?></option>
                              <?php
                            }
                          ?>
                        </select> 
                ...
                <?php } ?>
                                    ...
                                    <?php
                                if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Territory_sportclub!='' ){
									$teritorysportclub = '<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Territory_sportclub.'</p>';
								}else{
									$teritorysportclub='';
								}
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Territory!='' ){

									$territory= '<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Territory.'</p>';
								}else{
									$territory='';
								}  
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Selectclub!='' ){  

									$selectclub= '<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Selectclub.'</p>';  
								}else{
									$selectclub='';
								}
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Term_athlete!='' ){

									$termathlet= '<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Term_athlete.'</p>';
								}else{
									$termathlet='';
								}
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Agency_fee!='' ){
									$agencyfee= '<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Agency_fee.'</p>';
								}
								else{
									$agencyfee='';
								}
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Extension_fee!='' ){
									$extensionfee= '<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Extension_fee.'</p>';
								}else{
									$extensionfee='';
								}
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Type!='' ){
									$type='<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Type.'</p>';
								}else{
									$type='';
								}
								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Athlete_termination!='' ){
									$athletetermination='<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Athlete_termination.'</p>';
								}else{
									$athletetermination='';
								} 

								if(isset($mandate_histry) AND $mandate_check->_Status== '0' AND $mandate_histry->_Additional_terms!='' ){
									$additionalterms='<p class="nagotiationsmessage">Need to negotiation with- '.$mandate_histry->_Additional_terms.'</p>';
								}else{
									$additionalterms='';
								} 
                                     ?>		
                                        ...
                                        <select name="territoryselect[]" class="form-control select2" multiple="true" <?php echo $readonly; ?>>
                                        	...
                                        	if($countries){
                                        		foreach ($countries as $country) { ?>
                                        			<option <?php if(in_array(trim($country->countryname), $tarr) ){ ?> SELECTED <?php } ?> value="<?php echo $country->countryname; ?>"><?php echo $country->countryname; ?></option>
                                        		<?php
                                        		}
                                        	}  ?> 
                                        </select>
                                    ...
                                    <div class="col-sm-6 col-md-4 selectclubdiv"  style="<?php echo $dn; ?>" >
                                      <div class="form-group ">
                                        ...
                                        <select id="selectclub" name="selectclub" class="custom-select form-control" required <?php echo $readonly; ?>>
                                            <option value="">Select </option>
                                            <option value="Manchester United" <?php if( isset($mandate_check) AND $mandate_check->_Selectclub == 'Manchester United'){echo "SELECTED"; } ?>>Manchester United </option>
                                            <option value="Arsenal" <?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'Arsenal'){echo "SELECTED"; } ?>>Arsenal </option>
                                            <option value="Chelsea"<?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'Chelsea'){echo "SELECTED"; } ?>>Chelsea </option>
                                            <option value="Liverpool"<?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'Liverpool'){echo "SELECTED"; } ?>>Liverpool </option>
                                            <option value="Olympique Lyonnais" <?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'Olympique Lyonnais'){echo "SELECTED"; } ?>>Olympique Lyonnais </option>
                                            <option value="Olympique de Marseille" <?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'Olympique de Marseille'){echo "SELECTED"; } ?>>Olympique de Marseille </option>
                                            <option value="AS Monaco" <?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'AS Monaco'){echo "SELECTED"; } ?>>AS Monaco </option>
                                            <option value="Paris Saint-Germain" <?php if(isset($mandate_check) AND $mandate_check->_Selectclub == 'Paris Saint-Germain'){echo "SELECTED"; } ?>>Paris Saint-Germain </option>
                                          </select>
                                        <?php echo $selectclub;	?>
                                      </div>
                                    </div>
                                    <div class="col-sm-6 col-md-4">
                                      <div class="form-group">
			                            ...
			                            <?php  if(isset($mandate_check)){ $atharr=explode(' ',  $mandate_check->_Term_athlete); }else{$atharr=array(); } ?>
                                        <select id="term_athlete" class="custom-select form-control" required=""  name="term_athlete" <?php echo $readonly; ?> />
										<?php
			                                      for($i=1;  $i<30;  $i++) {
			                                        if(isset($mandate_check) AND $atharr[0] == $i){
			                                                 $selectm = "SELECTED";
			                                        }else{
			                                           $selectm = "";
			                                        }
									   ...
									   <span class="input-group-addon currency-addon dropdowon-box">
											...
											<option value="0" <?php if(isset($mandate_check) AND $atharr[1]=='Day'){echo "SELECTED"; } ?>>Day</option>
											<option value="1" <?php if(isset($mandate_check) AND $atharr[1]=='Month'){echo "SELECTED"; } ?>>Month</option>
										   </select>
										</span>
									   <?php echo $termathlet; ?>
                                          </div>
                                       ...
                                       <form action="<?php echo base_url() ?>myprofile/accept_mandate" method="post">
                                           ...
                                           <input type="hidden" name="histryid" value="<?php echo isset($mandate_histry)?$mandate_histry->_Histry_ID:''; ?>" />
                                             ...
                                             <?php } ?>
                                                         ...
                                                         <button name="negobtn" class="attributebtn negotcls btn btn-info negotiationsbtn" >Negotiations</button>
                                       </form>
                                <?php } ?>
...
jQuery("#selectagent").change(function(){
    window.location.href = "<?php echo base_url('player-mandate/') ?>"+jQuery(this).val();
})   
