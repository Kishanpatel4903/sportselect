...
public function newsignup(){
	//print_r($_FILES); print_r($_POST); exit;
			...
			$profile = date("dmYHis").$_FILES["cv"]["name"];
			...
			$data = array(
				...
				"_Password" => md5($this->input->post('password_mirror')),
			...
			if($query1>0){
					...
					if($this->input->post('type') == '1'){
						...
						$subdata = array(
							...
							"_Position" =>$this->input->post('position'),
					...
					if($query>0){
						...
						$maildata['mailed_data'] = array(
                                        ...
                                        'logo'    => '<i class="fa fa-cube"></i><b>Sport select</b>',//base_url().'assets/images/trader-logo.png',
	                    ...
	                    $subject = "Hi ".$this->input->post('firstname').", Welcome to Sport Select";

	                    $this->load->library('email', $config);
	                    $this->email->set_newline("\r\n");

	                    $this->email->from('support@sportselect.com','Sport Select');
	                    $this->email->to($email);
